...
namespace App\Repositories\Logistic\Report\Warehouse\CurrentStockDetail;
...
{
    public static function datatable($search, $dateStart, $dateEnd, $productIds, $categoryProductIds, $warehouseId)
    {
        // QUERY : LAST STOCK
        ...
        return Product::select(
            ...
            ->leftJoinSub($querySumTransaction, 'transactions', function ($join) {
                $join->on('stocks.product_id', '=', 'transactions.product_id')
                    ->on('stocks.company_id', '=', 'transactions.company_id')
                    ->on('stocks.warehouse_id', '=', 'transactions.warehouse_id')
