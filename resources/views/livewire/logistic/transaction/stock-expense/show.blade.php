    ...
    <form wire:submit="store">
        ...
        <div class="row">
            <div class="col-md-12 mb-4">
                <label>Catatan</label>
                <textarea class="form-control" cols="30" rows="4" wire:model="note" {{ !$is_enabled ? 'disabled' : ''}}></textarea>
            </div>
        </div>
            ...
            <div class="col-md-12 mb-3">
                ...
                <div class="col-md-12 mb-4">
                    <div class="w-100" wire:ignore>
                        <select id="select2-product" class="form-select" {{ !$is_enabled ? 'disabled' : ''}}>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="row my-2" wire:key="{{ $item['key'] }}">
                        <div class="col-auto d-flex mb-3">
                            <button type="button" class="btn btn-danger mx-auto align-self-end h-auto"
                                wire:click="removeDetail({{ $index }})" {{ !$is_enabled ? 'disabled' : ''}}>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" wire:model.blur="purchaseRequestProducts.{{ $index }}.unit_detail_id" {{ !$is_enabled ? 'disabled' : ''}}>
                        ...
                        <div class="col">
                            ...
                            <input type="text" class="form-control currency"
                                wire:model="purchaseRequestProducts.{{ $index }}.quantity" {{ !$is_enabled ? 'disabled' : ''}}/>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class='row'>
            <div class="col-md-12 mb-4">
                <label>Catatan</label>
                <textarea cols="30" rows="4" class="form-control @error('note') is-invalid @enderror" wire:model="approval_note" {{ !$is_enabled ? 'disabled' : ''}}></textarea>
            ...
            <div class="col-md-4 mb-4">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox"
                        wire:model.live="is_sequentially" {{ !$is_enabled ? 'disabled' : ''}}>
        ...
        <div class="row">
            ...
            <div class="col-md-12 mb-3">
                ...
                <div class="col-md-12 mb-4">
                    <div class="w-100" wire:ignore>
                        <select id="select2-user" class="form-select" {{ !$is_enabled ? 'disabled' : ''}}>
                        </select>
                    </div>
                </div>
                ...
                @foreach ($approvalUsers as $index => $item)
                    <div class="row my-2" wire:key="{{ $item['key'] }}">
                        <div class="col-auto d-flex mb-3">
                            <button type="button" class="btn btn-danger mx-auto align-self-end h-auto"
                                wire:click="removeApprover({{ $index }})" {{ !$is_enabled ? 'disabled' : ''}}>
                                <i class='fa fa-times'></i>
                            </button>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            @endif
                            <input type="text" class="form-control currency"
                                wire:model="approvalUsers.{{ $index }}.position" {{ !$is_enabled ? 'disabled' : ''}}/>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <button type="submit" class="btn btn-success mt-3" {{ !$is_enabled ? 'disabled' : ''}}>
